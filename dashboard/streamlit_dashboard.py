...
st.set_page_config(page_title="HIV TCE Dashboard",
                   layout="wide",
...
def get_session():
    return boto3.Session(
        ...
        aws_secret_access_key=st.secrets["hiv-project"]
...
def build_donut(df: pd.DataFrame):
    ...
    df_plot = df_plot.groupby('class_signature',
                              as_index=False)['tce_count'].sum()
    df_plot = df_plot.sort_values('tce_count', ascending=False)
    other_row = df_plot[df_plot['class_signature'] == 'Other']
    df_plot = pd.concat(
        [df_plot[df_plot['class_signature'] != 'Other'], other_row])

    colors = px.colors.qualitative.Prism
    color_map = {
        sig: colors[i % len(colors)]
        for i, sig in enumerate(df_plot[df_plot['class_signature'] != 'Other']
                                ['class_signature'])
    }
    color_map['Other'] = '#3a4155'

    legend_order = list(df_plot[df_plot['class_signature'] != 'Other']
                        ['class_signature']) + ['Other']
...
def build_line(df: pd.DataFrame):
    ...
    fig = px.line(df, x="baseline_year", y="tce_count", markers=True)
    fig.update_traces(line=dict(color='#60a5fa', width=2),
                      marker=dict(color='#60a5fa',
                                  size=6,
                                  line=dict(color='#0d0f14', width=1.5)))
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)',
                      plot_bgcolor='rgba(0,0,0,0)',
                      xaxis=dict(title="Baseline Year",
                                 title_font=dict(family='IBM Plex Mono',
                                                 size=10,
                                                 color='#475569'),
...
with col1:
    st.markdown(
        '<div class="chart-tile"><p class="chart-label">New Regimen Class Combinations</p>',
        unsafe_allow_html=True)
    with st.spinner(""):
        df_regimen = load_regimen_data(session)
    st.plotly_chart(build_donut(df_regimen),
...
with col2:
    st.markdown(
        '<div class="chart-tile"><p class="chart-label">TCE Frequency by Year</p>',
        unsafe_allow_html=True)
    with st.spinner(""):
        df_year = load_year_data(session)
    st.plotly_chart(build_line(df_year),
                    use_container_width=True,
                    config={"displayModeBar": False})
    st.markdown('</div>', unsafe_allow_html=True)
